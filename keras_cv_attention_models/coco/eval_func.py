...
class DecodePredictions(layers.Layer):
    ...
    def __nms_global__(self, bbs, ccs, labels, score_threshold=0.3, iou_threshold=0.5, soft_nms_sigma=0.5, max_output_size=100):
        ...
        return functional.gather(bbs, rr), functional.gather(labels, rr), tf.stop_gradient(nms_scores)
    ...
    def call(self, preds, input_shape=None, training=False, **nms_kwargs):
        ...
        if self.use_static_output:
            ...
            ##self.__decode_single__(preds[0], **self.nms_kwargs, input_shape=input_shape)
            """
            def mapped_function(pred):
                return self.__decode_single__(pred, **self.nms_kwargs, input_shape=input_shape)
            return tf.map_fn(mapped_function, preds)
            """
